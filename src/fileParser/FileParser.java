...
public class FileParser {
	...
	/**
	 * Initialises all public arrays used in this class to avoid null reference exceptions
	 */
	...
	/**
	 * Reads data in the specified input file into the array 'Lines' so that it can be extracted
	 */
	private void readLines(String fileName){
		...
		try {
			...
			System.out.println(String.format("Error: Could not open file '%s' ", fileName));
			System.exit(1);
		}
	...
	/**
	 * Extracts the data in the Lines array into proper types and variables so that
			...
			if (line.contains("router-id")) {
				...
				for(int i = 1; i < split.length; i++) {
					try{
						...
						System.out.println("Error: Incorrect router-id in configuration file");
						System.exit(0);
					}
				}
			}
			...
			if (line.contains("input-ports")) {
				...
				for(int i = 1; i < split.length; i++) {
					try{
						...
						System.out.println("Error: Incorrect input port in configuration file");
						System.exit(0);
					}
				}
			}
	...
	/**
	 * Extracts the output port information into the array list OutputPorts
	 * The output port information is represented like this
	 * 
					...
					try {
						...
						for(int i = 1; i < split.length; i++) {
							try {
								...
								OutputPortInformation info = new OutputPortInformation(portNumber, linkCost, routerId);
								OutputPorts.add(info);
								
							} catch (Exception e) {
								System.out.println("Error: Incorrect output in configuration file");
								System.exit(0);
							}
						}
